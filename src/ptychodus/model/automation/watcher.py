...
class DataDirectoryEventHandler(watchdog.events.FileSystemEventHandler):
    def __init__(self, dataset_buffer: PendingFileBuffer) -> None:
        super().__init__()
    ...
    def on_created_or_modified(self, event: watchdog.events.FileSystemEvent) -> None:
        if not event.is_directory:
            ...
            current_workflow = self._dataset_buffer.get_current_workflow()

            if src_path.match(current_workflow.get_watch_file_pattern()):
...
class DataDirectoryWatcher(Observable, Observer):
    def __init__(
        ...
        dataset_buffer.add_observer(self)
    ...
    def _update_watch(self) -> None:
        ...
        if data_directory.exists():
            ...
            observed_watch = self._observer.schedule(
                event_handler=DataDirectoryEventHandler(self._dataset_buffer),
                path=str(data_directory),
                recursive=current_workflow.is_watch_recursive,
            )
    ...
    def _update(self, observable: Observable) -> None:
        if observable is self._settings:
            self._update_watch()
        elif observable is self._dataset_buffer:
            self._update_watch()
