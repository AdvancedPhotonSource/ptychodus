...
def _create_confidential_client(client_id: str, client_secret: str) -> PtychodusClient:
...
def _create_native_client(
    code_handlers: Sequence[fair_research_login.CodeHandler],
) -> PtychodusClient:
...
def create_client(
    ...
    try:
        ...
        return _create_native_client(code_handlers)
    else:
        logger.info(
            'Environment variables "CLIENT_ID" and "CLIENT_SECRET" set. '
            'Creating confidential PtychodusClient.'
        )
        return _create_confidential_client(client_id, client_secret)
