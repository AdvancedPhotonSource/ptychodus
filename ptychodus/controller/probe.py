...
class ProbeProbeController(Observer):
    ...
    def _syncModelToView(self) -> None:
        ...
        self._view.wavelengthWidget.setLengthInMeters(
                self._presenter.getProbeWavelengthInMeters())
        self._view.diameterWidget.setLengthInMeters(
                self._presenter.getProbeDiameterInMeters())
...
class ProbeZonePlateController(Observer):
    ...
    def createInstance(cls, presenter: ProbePresenter, view: ProbeZonePlateView):
        ...
        view.zonePlateRadiusWidget.lengthChanged.connect(
                presenter.setZonePlateRadiusInMeters)
        ...
        view.beamstopDiameterWidget.lengthChanged.connect(
                presenter.setBeamstopDiameterInMeters)
...
class ProbeParametersController:
    def __init__(self, presenter: ProbePresenter, view: ProbeParametersView) -> None:
        ...
        self._zonePlateController = ProbeZonePlateController.createInstance(presenter, view.zonePlateView)
        self._initializerController = ProbeInitializerController.createInstance(presenter, view.initializerView)
    ...
    def openProbe(self) -> None:
        fileName, _ = QFileDialog.getOpenFileName(self._view,
                'Open Probe', str(Path.home()), ProbeIO.FILE_FILTER)
    ...
    def saveProbe(self) -> None:
        fileName, _ = QFileDialog.getSaveFileName(self._view,
                'Save Probe', str(Path.home()), ProbeIO.FILE_FILTER)
