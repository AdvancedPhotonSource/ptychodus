...
class FourierRingCorrelationViewController:
    ...
    def __init__(self, correlator: FourierRingCorrelator, treeModel: ObjectTreeModel,
                 parent: QWidget | None) -> None:
        super().__init__()
        self._correlator = correlator
        self._dialog = FourierRingCorrelationDialog.createInstance(parent)
        self._dialog.name1ComboBox.setModel(treeModel)
        self._dialog.name1ComboBox.textActivated.connect(self._redrawPlot)
        self._dialog.name2ComboBox.setModel(treeModel)
