...
Container builds: `scripts/podman/build` (Dockerfiles live in [containers/](containers/); produces `ptychodus:<version>-{cpu,cuda12.8,cuda13.0,cuda13.2,rocm7.2.4,xpu}`); launcher `scripts/podman/ptychodus` auto-detects GPU. Docs: `make -C docs html`.
...
Tests cover `api/`, `model/`, and `view/widgets/`, plus the two modules outside those trees that carry real logic rather than plain Qt wiring: [controller/parameters.py](src/ptychodus/controller/parameters.py) (unit conversion, unit selection, and signedness inference in its view controllers) and `DecimalRangeSlider` in [view/image.py](src/ptychodus/view/image.py) (clamping, tick arithmetic, handle ordering, edge-triggered signals). **Do not add tests for anything else under `view/` or `controller/`**, including controller helper functions. Qt tests depend on PyQt5, which is an optional extra (`--extra gui`); the whole `tests/view/` and `tests/controller/` subtrees are dropped in [tests/conftest.py](tests/conftest.py) via `collect_ignore` when `find_spec('PyQt5')` returns `None`, mirroring the `ptychodus_store` gate. That conftest also hosts the shared session-scoped `qapp` fixture, which imports `QApplication` inside the fixture body so the conftest itself stays importable on a bare install. `pytest tests/` must pass on a bare `pip install .` (that is what CI runs), so anything reachable from `ptychodus.model` at import time must be free of optional dependencies: gate optional backends with the `find_spec` probe + deferred factory import used in each `model/*/core.py`. Guard a test that needs an optional backend with `pytest.importorskip('ptychi')`; drop a whole optional-extra directory with `collect_ignore` in [tests/conftest.py](tests/conftest.py) — `importorskip` in a conftest is reported as an error, not a skip.
...
- **Underscore-prefixed names in `api/`.** A name is public iff it is used from outside `api/` (from `model/`, `view/`, `controller/`, `tests/`, `plugins/`, `scripts/`, or `ptychodus_store/`) **or** it is listed in the module's `__all__` as a deliberate publish decision. Everything else — including helpers used only cross-module within `api/` — is `_`-prefixed. When a private helper needs to cross module lines within `api/`, refactor the caller so the helper stays module-local (typically by moving the logic onto a peer method of the shared abstract base, as `BinningStep.apply_to_dtype` does for what would otherwise be a cross-module private helper) rather than reaching into another module's underscore names. Rename on promotion when a helper first grows an external consumer; the audit is `grep -rEn 'from ptychodus\.api\.[a-z_]+ import _' src/ tests/ scripts/`.
- **Pipeline steps declare their dtype effect.** Any `DiffractionPrepStep` whose `apply()` widens value range (summing, scaling, accumulating) must override `apply_to_dtype(dtype)` to return a dtype that holds the promoted result, and its `apply()` must produce arrays of that dtype (typically `numpy.sum(..., dtype=self.apply_to_dtype(data.dtype))`). This is the sister method of `apply_to_extent` and `apply_to_pixel_geometry`; the default is identity. Callers that allocate a pre-sized buffer (`allocate_assembled_data` and any future analogue) fold it via `DiffractionPrepPipeline.compute_output_dtype(pattern_dtype)` so the buffer is wide enough by construction. Silent-truncation traps (accumulating into a wider intermediate then narrowing on assignment into an exogenous-dtype buffer) are prevented by this contract, not by ad-hoc `isinstance` checks in the allocator.
- **Classmethods vs free functions in `api/`.** Alt-constructors on domain types use `@classmethod` with the naming split `Cls.from_X(x, ...)` for data-taking factories (`Interval.from_bounds` — following the stdlib `dict.fromkeys` / `datetime.fromtimestamp` idiom) and `Cls.create_X()` for parameterless factories of well-known instances (`DiffractionMetadata.create_null`, `ReconstructionAmbiguities.create_identity`, `SimpleTreeNode.create_root`). Domain-verb names (`PluginRegistry.load_plugins`) are reserved for factories with a meaningful side effect that `from_*` doesn't convey. `@staticmethod` inside classes is avoided: helpers that touch neither `self` nor `cls` go to module scope (prefixed with `_` when private). Per-`Product` (or per-domain-object) analyzers that return a small dataclass live as free `compute_*` / `estimate_*` functions in the activity-verb module — [metrics.py](src/ptychodus/api/metrics.py), [preprocess/](src/ptychodus/api/preprocess/), [reconstruct.py](src/ptychodus/api/reconstruct.py) — even when the result dataclass itself is defined elsewhere; the reference pattern is `compute_fourier_ring_correlation`, `compute_object_comparison`, `estimate_reconstruction_ambiguities`, `compute_reconstruction_residuals`.
- The `ptychodus_store/ui/` frontend is TypeScript compiled to native ES modules with plain `tsc` — no bundler, no framework, no runtime npm deps. The compiled `ui/dist/` is gitignored but ships in both the sdist and the wheel via `[tool.setuptools.package-data]`, so installing from PyPI never needs Node. [setup.py](setup.py) compiles it from **both** the `sdist` and `build_py` cmdclasses — `sdist` finalizes `build_py` to collect package data but never runs it, so hooking only `build_py` leaves the tarball without a UI. A missing `tsc` warns and builds without the UI rather than failing; set `PTYCHODUS_STORE_REQUIRE_UI_BUILD=1` (release builds do) to make a missing or stale UI fatal. Interactive dev needs `tsc` on PATH.
